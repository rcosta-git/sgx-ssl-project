...
static sgx_status_t SGX_CDECL sgx_t_gen_keys(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_t_gen_keys_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_t_gen_keys_t* ms = SGX_CAST(ms_t_gen_keys_t*, pms);
	sgx_status_t status = SGX_SUCCESS;
	unsigned char* _tmp_pbuf = ms->ms_pbuf;



	ms->ms_retval = t_gen_keys(_tmp_pbuf);
